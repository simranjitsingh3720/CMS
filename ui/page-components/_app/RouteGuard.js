...
function RouteGuard({ children, session }) {
  ...
  useEffect(() => {
    if (session.user) {
      if (router.pathname.includes('/admin/signup') || router.pathname.includes('/admin/signin') || router.pathname.includes('/admin/password-recovery') || router.pathname.includes('/admin/password-change')) {
